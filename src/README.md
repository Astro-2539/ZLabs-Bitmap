...
**如需对字体进行二次创作，建议您修改此文件夹中的文件。**

### ZLabsPixel_12px_M_CN.kbitx

该文件为中国大陆变体字形（CN）的源工程文件，亦是其他变体字形的基础文件。

该文件存储了系列字体的所有西文字符、符号及所有的陆标汉字字符。

### ZLabsPixel_12px_M_HC_diff.kbitx / ZLabsPixel_12px_M_JP_diff.kbitx

该文件为中国香港变体字形（HC）/日本变体字形（JP）的源工程文件之一。

该文件**仅存储**与中国大陆字形有差异的汉字及符号，不能单独生成字体。

需要配合`ZLabsPixel_12px_M_CN.kbitx`及`flags_HC.txt`/`flags_JP.txt`以生成完整字体。
